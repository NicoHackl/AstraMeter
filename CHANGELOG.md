...
- **Added** **OBI Energy Tracking** (heyOBI) as a power source (`[OBI_ENERGY]`): sign in with your heyOBI account and AstraMeter puts the sensor into the app's live mode, so grid readings arrive roughly every two seconds instead of every few minutes. The sensor is returned to its normal reporting interval on shutdown to spare its batteries. See [docs/powermeters.md](docs/powermeters.md#obi-energy-tracking)
