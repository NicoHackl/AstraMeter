...
- **Fixed** a battery without a committed phase reference (polling in "inspection mode", e.g. a Marstek B2500 whose phase self-check never completed) silently crippling active control for every other battery: it soaked up most of the load allocation without ever being steerable, which could pin the remaining battery at its minimum output while the house kept importing. Such batteries no longer take a share of the load, and they are now served unfiltered meter readings so the phase self-check they need to escape inspection mode has a real signal to work with ([#559](https://github.com/tomquist/astrameter/issues/559)).
