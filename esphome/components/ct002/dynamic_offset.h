...
// Adds a single, runtime-adjustable offset (watts) to every phase. Mirrors
// src/astrameter/powermeter/wrappers/dynamic_offset.py.
//
...
// the "Grid Offset" HA Number. It shifts the value the control loop / balancer
// see; the raw reading (get_powermeter_watts_raw, used by the Marstek app and
// cloud reporting) passes through untouched so those consumers still match the
// physical meter.
 ...
 public:
  ...
  std::vector<float> get_powermeter_watts() override {
    ...
    if (this->offset_ != 0.0f) {
      for (float &v : values) v += this->offset_;
    }
    return values;
  }
