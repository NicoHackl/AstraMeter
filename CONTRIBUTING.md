...
- `src/astrameter/powermeter/wrappers/dynamic_offset.py` → `esphome/components/ct002/dynamic_offset.h` (header-only). The *wrapper* (a live per-phase additive offset) is mirrored, but its position and control surface differ per stack: Python inserts it in the powermeter pipeline and drives it from the **Python-only** `{base}/powermeter/<section>/offset/set` topic; ESPHome inserts it at the head of the ct002 filter pipeline (`CT002Component::set_grid_offset`) and drives it from the ct002 **device** command `{"grid_offset": <w>}` + the HA "Grid Offset" number (see the discovery/service exceptions below). Both add on top of the static offset (`POWER_OFFSET` / a native `sensor: filters: offset:`).
