...
- `src/astrameter/ct002/ct002.py` → `ct002.{h,cpp}` (including the response-builder math, MAC validation, and `_compute_smooth_target` dispatch). `ct_mac_advertise` / `set_ct_mac_advertise` is a **code-shape mirror only**: it decides the CT MAC a response carries when the request names none (an unpaired battery probes with the all-zero `WILDCARD_CT_MAC`, which is not selectable in the Marstek app), and it is deliberately kept apart from `ct_mac`, which *gates* which batteries are answered. No `ct002:` YAML key sets it — the firmware's `marstek_registration` writes the registered MAC straight into `ct_mac_` — so the ESPHome side is behaviourally unchanged and `test_shared_e2e.py` cannot drive that branch. Port edits to the precedence chain by inspection, keeping the three cases in the same order on both sides.
